classdef NewClassStrategyResult3 < mlsystem_unittest.NewClassStrategyResult2
	%% NEWCLASSSTRATEGYRESULT3 is the output result of NewClassStrategy that has one superclass
	%
	%  $Revision$ 
	%  was created $Date$ 
	%  by $Author$, 
	...
	methods 
		% N.B. (Abstract, Access='protected', Hidden, Sealed, Static) 
 		...
 		function this = NewClassStrategyResult3(varargin) 
			...
			%  Usage:   this = NewClassStrategyResult3([parameter_name, parameter_value, ...]) 

			this = this@mlsystem_unittest.NewClassStrategyResult2(varargin{:});
		end 
 	end 
