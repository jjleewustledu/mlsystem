classdef DirTool2
    ...
    methods (Static)
        ...
        function [S,R] = rm(ca, flags)
            ...
            if (DirTool2.hasflags__(ca)); [ca,flags] = DirTool2.swap__(ca, flags); end     
            ca = ensureCell(ca);
            try
        ...
        function [S,R] = cp(ca, dest, flags)
            ...
            if (~exist('flags','var'));        flags  = ''; end        
            if (DirTool2.hasflags__(ca));   [ca,  flags] = DirTool2.swap__(ca,   flags); end  
            if (DirTool2.hasflags__(dest)); [dest,flags] = DirTool2.swap__(dest, flags); end 
            ca = ensureCell(ca);
            try
	...
	methods 
        ...
        function ff   = fqfns(this, idx)
            ...
            for s = 1:length(ff)
                ff{s} = mlsystem.DirTool2.fqname__(sarr(s));
            end
            if (exist('idx','var'))
                ff = ff{idx}; 
            else
        ...
        function ff   = fns(  this, idx)
            ...
            ff   = cell(1,length(sarr));
            for s = 1:length(ff)
                ff{s} = mlsystem.DirTool2.name__(sarr(s));
            end
            if (exist('idx','var'))
                ff = ff{idx}; 
            else
        ...
        %% ctor
 		function this = DirTool2(varargin) 
 			%% DirTool2 
            ...
            this.pwd_ = pwd;
            this.itsPath_ = this.setItsPath__(this.name_);
 		end 
    end 
    ...
    methods (Access = 'private', Static)
        ...
        function ff    = fqname__(strct)
            ff = fullfile(strct.folder, strct.name);
        end
    ...
    methods (Access = 'private')
        ...
        function sarr = includePath__(~, sarr)
            for s = 1:length(sarr) %#ok<FORFLG>
                sarr(s).itsPath = sarr(s).folder;
            end
        end
        function pth  = setItsPath__(this, pth)            
            ...
            error('mlsystem:ioError', 'DirTool2.setItsPath__');
        end
